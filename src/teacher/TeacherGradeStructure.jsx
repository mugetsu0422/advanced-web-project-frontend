...
const TeacherGradeStructure = () => {
  ...
  const fetchData = () => {
    axios.get(`${import.meta.env.VITE_SERVER_HOST}/teachers/class/${id}/grade-compositions`,
      {
        headers: {
    ...
    .then((response) => {
      const sortedCompositions = response.data.sort((a, b) => a.order - b.order);
      setGradeCompositions([...sortedCompositions]);
      const copyOfSortedCompositions = JSON.parse(JSON.stringify(sortedCompositions));
      setOriginalGradeCompositions(copyOfSortedCompositions);
    })
    .catch((err) => {
      showAlertFunction(err.response.data.message, 'danger');
    })
    const copyOfGradeCompositions = JSON.parse(JSON.stringify(gradeCompositions));
  ...
  const handleAddRow = () => {
    const newComposition = { id: gradeCompositions.length + 1, name: 'Assignment', scale: '0' };
        ...
        <tbody>
          {gradeCompositions.map((composition, index) => (
            ...
            >
              <td>
                {editingIndex === index ? (
                  <Input className={styles['input']} value={composition.name} onChange={(e) => handleNameChange(e, index)} />
                ) : (
                  composition.name
                )}
              </td>
              <td>
                {editingIndex === index ? (
                  <Input className={styles['input']} value={composition.scale} onChange={(e) => handleGradeScaleChange(e, index)} />
                ) : (
                  composition.scale
                )}
              </td>
              <td className={styles['operation-cell']}>
                {editingIndex === index ? (
                  <>
                    <Button className={styles['edit-button']} onClick={() => handleSaveTemporaryChanges(index)}>
                      <SaveOutlined />
                      Save
                    </Button>
                  ...
                  <>
                    <Button className={styles['edit-button']} onClick={() => handleEdit(index)}>
                      <EditOutlined />
                      Edit
                    </Button>
